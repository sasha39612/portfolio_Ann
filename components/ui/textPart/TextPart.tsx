...
const TextPart = ({
  ...
  const subTitleStyle = cn(
    subTitle?.className ? `${styles[subTitle.className]}` : '',
    className,
  );
  const descriptionStyle = !Array.isArray(description)
    ? cn(
        description?.className ? `${styles[description.className]}` : '',
        className,
      )
    : '';
      ...
      {Array.isArray(description) ? (
        <ul>
          {description?.map((item: TextBlockElemArr) => (
            <li key={item.id}>
              <p className={styles[item?.className ?? '']}>
                {item?.text}
                {item?.image?.src ? (
                  <span
                    onClick={(e) => handleOnClick(e, item.popupContent)}
                    className={getIconStyles(item?.image?.className)}
                    style={getIconContent(item?.image?.src)}
                  />
                ) : null}
              </p>
            </li>
        ...
        <>
          <p className={descriptionStyle}>
            ...
            {description?.image?.src ? (
              <span
                onClick={(e) => handleOnClick(e, popupContent)}
                className={getIconStyles(description?.image?.className)}
                style={getIconContent(description?.image?.src)}
              />
            ) : null}
          </p>
        </>
